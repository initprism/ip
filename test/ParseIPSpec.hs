...
spec_buildIP =
    describe "buildIP" $ do
        ...
        it "builds from arbirary address" $
...
spec_parseIP = 
    describe "parseIP" $ do
        ...
        it "parses some random IP address" $
            parseIP "192.168.3.15" `shouldBe` Just (buildIP [192,168,3,15])
        it "fails to parse 3 componets"  $
            parseIP "192.168.1" `shouldBe` Nothing
        it "fails to parse 4 coponents with suffix" $
        ...
        it "fails to parse extremely large componets" $
...
spec_parseIPRange = 
    describe "parseIPRange" $ do
        it "parses pair" $ 
            parseIPRange "192.168.0.1,192.168.3.100" `shouldBe` IPRange <$> parseIP "192.168.0.1" <*> parseIP "192.168.3.100"
...
spec_parseIPRanges =
    describe "parseIPRanges" $ do
        let sample_range1 = "192.168.0.1,192.168.3.100"
            sample_range2 = "0.0.0.0,0.0.0.1"
        it "parses empty list" $
            parseIPRanges "" `shouldBe` Right (IPRangeDB [])
        it "parses single range" $
            parseIPRanges sample_range1 `shouldBe` Right (IPRangeDB $ catMaybes [parseIPRange sample_range1])
        it "parses two ranges" $
            parseIPRanges (sample_range1 ++ "\n" ++ sample_range2) `shouldBe`
                Right (IPRangeDB $ catMaybes [parseIPRange sample_range1, parseIPRange sample_range2])
